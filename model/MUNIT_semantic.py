...
class MUNIT_semantic(BaseModel):
    ...
    def sample(self):
        ...
        image_a = (self.image_a.cpu() + 1.0) / 2.0
        image_b = (self.image_b.cpu() + 1.0) / 2.0
        saved_image = [image_a, image_b]
        with torch.no_grad():
            # encode
            s_a, c_a = self.gen_a.encode(self.image_a)
            s_b, c_b = self.gen_b.encode(self.image_b)
            ...
            # decode (reconstruction)
            image_a_rec = (self.gen_a.decode(s_a, c_a).cpu() + 1.0) / 2.0
            image_b_rec = (self.gen_b.decode(s_b, c_b).cpu() + 1.0) / 2.0
            saved_image += [image_a_rec, image_b_rec]

            # decode (translation)
            image_a_fake1 = (self.gen_a.decode(self.s_a_rnd, c_b).cpu() + 1.0) / 2.0
            image_b_fake1 = (self.gen_b.decode(self.s_b_rnd, c_a).cpu() + 1.0) / 2.0
            image_a_fake2 = (self.gen_a.decode(s_a_rnd, c_b).cpu() + 1.0) / 2.0
            image_b_fake2 = (self.gen_b.decode(s_b_rnd, c_a).cpu() + 1.0) / 2.0
            saved_image += [image_b_fake1, image_a_fake1, image_b_fake2, image_a_fake2]

            # prediction of fake image
            pseudo_label_a = torch.max(self.classification(self.image_a), dim=1)[1]
            pseudo_label_b = torch.max(self.classification(self.image_b), dim=1)[1]
            label_fake_a = torch.max(self.classification(image_a_fake1.cuda()), dim=1)[1]
            label_fake_b = torch.max(self.classification(image_b_fake1.cuda()), dim=1)[1]
